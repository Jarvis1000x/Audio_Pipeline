...
def create_pipline(transformations: list):
    pipeline = []
    for index, transformations in enumerate(transformations):
        if transformations:
...
def create_audio_player(audio_data, sample_rate):
    ...
    wavfile.write(virtual_file, rate=sample_rate, data=audio_data)
    return virtual_file
...
def plot_audio_transformation(y, sr, pipeline: audiomentations.Compose):
    cols = [1, 1, 1]

    col1, col2, col3 = st.columns(cols)
    with col1:
        st.markdown(f"<h4 style='text-align: center;'>Original</h5>",
                    unsafe_allow_html=True)
        st.pyplot(plot_transformation(y, sr, "Original"))
    with col2:
        st.markdown(f"<h4 style='text-align: center;'>Wave Plot</h5>",
                    unsafe_allow_html=True)
        st.pyplot(plot_wave(y, sr))
    with col3:
        st.markdown(f"<h4 style='text-align: center;'>Audio</h5>",
                    unsafe_allow_html=True)
        spacing()
    ...
    for col_index, individual_transformation in enumerate(pipeline.transforms):
        ...
        with col1:
            st.markdown(f"<h4 style='text-align: center;'>{transformation_name}</h5>",
                        unsafe_allow_html=True)
            st.pyplot(fig)
        with col2:
            st.markdown(f"<h4 style='text-align: center;'>Wave Plot</h5>",
...
def action(file_uploader, selected_provided_file, transformations):
    ...
    pipeline = audiomentations.Compose(create_pipline(transformations))
    plot_audio_transformation(y, sr, pipeline)


def main():
    placeholder = st.empty()
    placeholder2 = st.empty()
    placeholder.markdown("# Audio Preprocessing Pipeline for Machine Learning\n"
                         "## Visualising an Audio Pipeline\n"
                         "### Select the components of the pipeline in the sidebar.\n"
                         "Once you have chosen augmentation techniques, select or upload an audio an audio file.\n"
                         "Then click \"Apply\" to start\n")
    placeholder2.markdown(
        "After clicking start, the individual steps of the pipeline are visualised. "
        "The output of the previous step is the input to the next step"
    )
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("Upload the audio file here:")
    file_uploader = st.sidebar.file_uploader(label="", type=[".wav", ".wave", ".flac", ".mp3", ".ogg"])
    st.sidebar.markdown("Or select a sample file:")
    select_provided_file = st.sidebar.selectbox(label="", options=["Cow", "Dog", "Thunder"])
    ...
    if st.sidebar.button("Apply"):
        ...
        action(file_uploader=file_uploader, selected_provided_file=select_provided_file,
               transformations=transformations)


if __name__ == "__main__":
    st.set_page_config(layout="wide", page_title="Audio Augmentation Visualisation")
    main()
